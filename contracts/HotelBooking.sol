...
contract HotelBooking {
    ...
    IERC20 public token;
    address public tokenContractAddress; // 添加代币合约地址变量
    ...
    constructor(address _token) {
        owner = msg.sender;
        token = IERC20(_token);
        tokenContractAddress = _token; // 初始化代币合约地址
    }
    ...
    // 调用 `HotelToken` 合约的 `buyTokens` 函数，允许顾客用ETH购买代币
    function buyTokens() public payable {
        require(msg.value > 0, "Must send ETH to buy tokens");
        (bool success, ) = tokenContractAddress.call{value: msg.value}(
